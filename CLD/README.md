...
1. `flr_config`: the configuration of `Flr` which is specified in `pubspec.yaml` by `Flr`. 
   ...
   -  `dartfmt_line_length`：用于存放`Flr`工具的CLI版本和VSCode Extension版本对`r.g.dart`进行格式化时的行长，初始值为：80（dartfmt工具的默认值）；后期用户可根据团队需要进行手动修改
...
2. 判断父目录是否符合资产变体目录的特征
   ...
   - 倍率变体目录特征：`^((0\.[0-9]+)|([1-9]+[0-9]*(\.[0-9]+)?))[x]$`，比如`2x`、`2.0x`。
